...
export const lengthOfLongestSubstring = function(str: string): number {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    while (storeLength--) {
      // Loop through the store and add to our old strings.
      const storeItem = store[storeLength];
      if (!storeItem.set.has(currentLetter)) {
        // if the store item doesn't have the item just add it and inc length
        storeItem.length++;
        storeItem.set.add(currentLetter);
      } else {
        // if it does have the item store the max length and delete it.
